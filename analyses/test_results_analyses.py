# coding=utf-8
# 当在一个文件夹下有一种细胞对于其他许多细胞的比较结果（MAmotif结果），我们想看某个motif的检验结果中有显著差异的结果数占所有
...
def show_one_motif_test_result(folder_of_comparisons, test_file_name, motif_name, output_file_name):
    ...
    test file name is a file in each comparisons. all comparison folder have the same name of this file
    """
...
def show_one_motif_fisher_test_result(folder_of_comparisons, test_file_name, motif_name, output_file_name):
    """
    @param test_file_name:
    test file name is a file in each comparisons. all comparison folder have the same name of this file
    """
...
def LICR(his, rep, jaspar3_name):
    for motif_name in jaspar3_name:
        folder_of_comparisons = '/mnt/MAmotif.py/2.Processing/4.Histone_LICR_mm9_MAmotif/%s_%s_MAmotif' % (his, rep)
        test_file_name = 'Esb4_%s_LICR_%s_promoter_peak_MAmotif_jaspar_output.xls' % (his, rep)
        output_file_name = 'Esb4_%s_LICR_%s_%s_promoter_MAmotif_result.xls' % (his, rep, motif_name)
        show_one_motif_test_result(folder_of_comparisons, test_file_name, motif_name, output_file_name)

        test_file_name = 'Esb4_%s_LICR_%s_distal_peak_MAmotif_jaspar_output.xls' % (his, rep)
        output_file_name = 'Esb4_%s_LICR_%s_%s_distal_MAmotif_result.xls' % (his, rep, motif_name)
        show_one_motif_test_result(folder_of_comparisons, test_file_name, motif_name, output_file_name)


def Epi(his, rep, jaspar3_name):
    for motif_name in jaspar3_name:
        folder_of_comparisons = '/mnt/MAmotif.py/2.Processing/4.Histone_Epi_hg19_MAmotif/%s_%s_MAmotif_Epi' % (his, rep)
        if his == 'H3K27ac':
            test_file_name = 'H1hesc_H3K27ac_SAK270_Top25K_promoter_peak_MAmotif_jaspar_output.xls'
        else:
            test_file_name = 'H1hesc_H3K9ac_SAK68_promoter_peak_MAmotif_jaspar_output.xls'
        output_file_name = 'H1hesc_%s_Epi_%s_%s_promoter_MAmotif_result.xls' % (his, rep, motif_name)
        show_one_motif_test_result(folder_of_comparisons, test_file_name, motif_name, output_file_name)

        if his == 'H3K27ac':
            test_file_name = 'H1hesc_H3K27ac_SAK270_Top25K_distal_peak_MAmotif_jaspar_output.xls'
        else:
...
def test_show_one_motif_test_result():
    ...
    for rep in reps:
        for motif in motifs:
            fd = '/mnt/MAmotif/2.Processing/8.Broad_Esb4_and_H3K4me3_MAmotif/4.MAmotif'
