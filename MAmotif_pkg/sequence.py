...
class Tag(Sequence):
    """
    Tag is aligned sequence after 2nd sequencing. we often calculate its point position.
    """
    def __init__(self, chrm, start, end, strand):
        Sequence.__init__(self, chrm, start, end, strand)

    def pos(self, shift_size):
        if self.strand == '+':
            return self.start + shift_size
        elif self.strand == '-':
            return self.end - shift_size
...
if __name__ == '__main__':
    seq1 = Sequence('chr1', 123, 342)
    seq2 = Sequence('chr3', 563, 765)
    seq3 = Sequence('chr5', 12343, 124122)
    seq4 = Sequence('chry', 123, 3412)
    seq5 = Sequence('chr15', 321, 1235)
    seqs = [seq2, seq1, seq4, seq3, seq5]
    set = Sequences()
    set.set_sequences(seqs)
    set.sort_sequences()
    for seq in set:
        seq.prints()
    pass