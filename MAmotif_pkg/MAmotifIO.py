...
def read_MAnorm_peaks(file_path, neg=False):
    ...
    """
    print '@info: reading MAnorm peaks of %s' % file_path.split(os.sep)[-1]
    pk_num = 0
    handle = open(file_path)
    pks = []
    for info in handle:
...
def read_bed_peaks(fp):
    print '@info: reading bed peak file %s' % os.path.basename(fp)
    pks = []
    with open(fp) as fi:
        for line in fi:
            cdt = line.split()
...
def read_motifscan_peaks(motifscan_file_path, motif_name):
    ...
    """
    print '@info: Reading MotifScan peaks for motif %s' % motif_name
    ...
    if motifscan_file_path.endswith('.mat'):  # from shao Matlab MotifScan result
        ...
        index = np.where(np.array(motifs)==motif_name)[0][0]
...
def output_genes_promoter(genes, fn):
    fo = open(fn, 'w')
    for gene in genes:
        promoter = gene.promoter_zone
        line = '\t'.join([promoter.chrm, str(promoter.start), str(promoter.end)]) + '\n'
        fo.write(line)
    fo.close()
...
def read_motifscan_result(motifscan_file_path):
    """
    read motifscan result with all jaspar3 motifs
    (not specific motif like MAmotif_pkg.MAmotifIO.read_motifscan_result)
    这个方法是一次性读取所有jaspar3的motifs的target number结果，
    而不是在MAmotif_pkg中只读取指定的一个motif的target_number结
    果，这样可以只做一次匹配（匹配MAnorm peaks和Motifscan的结果）
    而将所有peaks按照有没有某个motif进行分类。
    @param motifscan_file_path: motifscan result from Wangjiawei's peak result file
    @return: motifscan_peak_list, motifs
    """
    print 'reading motifscan result: %s, start!' % motifscan_file_path.split(os.sep)[-1]
    motifscan_peak_list = []
...
def match_manorm_with_motifscan(pk_file, motifscan_result, neg_mvalue=False):
    ...
    print '@info: MAnorm peaks num: %d' % len(manorm_pks)

    # read motifscan result
    motifscan_pks, motifs = read_motifscan_result(motifscan_result)

    # matching ...
    print '@info: match MAnorm peaks with motifscan result...'
    matched_manorm_pks = []
    matched_tarnum_list = []
    match_num = 0
...
def output_peaks_target_genes(pk_fp, refgene_fp):
    """
    获取输入的MAnorm peak文件中每一个peak的靶基因，一个peak可以有多个靶基因，
    多个peak也可能有共同的靶基因
...
def classify_MAnorm_pks_by_promoter(pk_fp, refgene_fp):
    ...
    print '@info: start to classify peaks by promoter zone ...'
    classifier.classify_by_feature()
    promoter_pkset = classifier.feature_yes
    print '@info: peak number in promoter: %d' % promoter_pkset.size
    distal_pkset = classifier.feature_no
    print '@info: peak number in distal: %d' % distal_pkset.size
...
def classify_MAnorm_peaks_by_motif(pks_fp, motifscan_fp, motif):
    ...
    print '@info: start classify peaks by %s existence' % motif
    ...
    classifier = MAnormPeaksClassifier(pkset, feature)
    classifier.classify_by_feature()
    print '@info: there are %d mismatch!' % feature.mismatch
    ...
    file_yes.write('\t'.join(
        ['#chr', 'start', 'end', 'summit', 'MAnorm-Mvalue', 'MAnorm-Avalue', 'p-value\n']))
...
def classify_MAnorm_pks_by_pvalue(pk_fp, pvalue=0.001):
    """
    MAnorm的结果给每个M值都对应了一个pvalue用来衡量变化的显著性，p值越小说明变化越显著，
    利用给定的p值将一组peaks分成两类
    """
    # get peaks
...
def motif_classified_pks_ttest(
    ...
    test_result = \
        open(pk_file_name[:-4].replace('_MAvalues', '') + '_MAmotif_jaspar_output' + '.xls', 'w')
    header = _get_header(correction_type)
    test_result.write(header)
...
def test_classify_MAnorm_pks_by_promoter():
    ...
    # classify_MAnorm_pks_by_promoter(
    #     'Ese14_H3K27ac_LICR_Rep2_peaks_MAvalues.xls', mm9_refgenes_file)


def test_classify_MAnorm_pks_by_motif():
    os.chdir('/mnt/MAmotif/3.Analysis/2.mm9KeyPeaksTFsEnrich/Ese14')
    motifs = ['Sox2', 'Pou5f1', 'ZNF263', 'MZF1_5-13']
    for motif in motifs:
        motifscan_fp = '/mnt/MAmotif/2.Processing/3.ESC_motif_scan_result/' \
                       'Ese14AndEScj7MotifScanRes_Shao/' \
                       'Ese14_H3K27ac_LICR_Rep2_jaspar3_motif_target_p0.0001.mat'
        classify_MAnorm_peaks_by_motif(
            'Ese14_H3K27ac_LICR_Rep2_stringent_distal_peaks_MAvalues.xls', motifscan_fp, motif)
...
def test_output_genes_promoter():
    os.chdir('/mnt/MAmotif/2.Processing/3.MotifScan_refseq_promoter')
    refseq_fp = '/home/zhaohui/MAmotif/data/refGene_mm9.txt'
    genes = read_refgenes(refseq_fp)
    fn = 'refGene_mm9_promoter.bed'
    output_genes_promoter(genes, fn)
...
if __name__ == '__main__':
    ...
    # test_classify_MAnorm_pks_by_motif()
    test_output_genes_promoter()
